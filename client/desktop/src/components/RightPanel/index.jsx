...
// UI
import { Panel, Logo, Login, Space, Item, Image, Remove, Count, More, Cart, Total } from "./ui";
        ...
        <Panel>
            ...
            <Space open={open}>
                {data.map((item) => {
                    return (
                        <Item key={item.id}>
                            <Tooltip position="left" title={`${item.title} (${item.count} x ${item.price}₽)`}>
                                <Image
            ...
            <Cart total={total} onClick={!open ? openCart : null}>
                <Tooltip position="left" distance="25" title={`${open ? "Зыкрыть" : "Открыть"} корзину`}>
                    <Total>{total}₽</Total>
                    <svg xmlns="http://www.w3.org/2000/svg">
                        <use xlinkHref="#cart" />
                        <rect width="100%" height="100%" style={{ fill: "transparent" }} />
